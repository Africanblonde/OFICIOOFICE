...
-- IMPORTANT NOTES:
-- 1. This schema file uses 'create type if not exists' to avoid duplicate type errors
-- 2. For the critical fix to handle_new_user function, apply the separate migration:
--    → See: migration_fix_handle_new_user.sql (applies DROP + RECREATE)
-- 3. RLS is disabled on public.users table due to Auth/trigger interactions
-- 4. To initialize fresh: apply this file, then apply migration_fix_handle_new_user.sql

-- ENUMS
-- Note: Create only if they don't exist. Supabase may already have these.
create type if not exists user_role as enum ('ADMIN', 'GENERAL_MANAGER', 'MANAGER', 'WORKER');
create type if not exists request_status as enum ('PENDING', 'APPROVED', 'REJECTED', 'IN_TRANSIT', 'DELIVERED', 'CONFIRMED');
create type if not exists location_type as enum ('CENTRAL', 'BRANCH', 'FIELD');
create type if not exists attendance_status as enum ('D', 'D/2', 'F');
create type if not exists item_type as enum ('CONSUMABLE', 'ASSET');
create type if not exists item_condition as enum ('NEW', 'GOOD', 'FAIR', 'POOR', 'DAMAGED');
create type if not exists transaction_type as enum ('SALE', 'EXPENSE');
create type if not exists payment_method as enum ('CASH', 'CARD', 'MOBILE_MONEY', 'BANK_TRANSFER', 'CHEQUE');
create type if not exists document_type as enum ('COTACAO', 'PRO_FORMA', 'FATURA', 'FATURA_RECIBO');
create type if not exists invoice_status as enum ('RASCUNHO', 'EMITIDA', 'PAGA_PARCIAL', 'PAGA', 'CANCELADA');
create type if not exists currency_code as enum ('MZN', 'USD');
...
-- Note: RLS temporarily disabled on users due to Auth/trigger interactions during user creation.
