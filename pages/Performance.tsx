...
export const Performance = () => {
  ...
  const renderDailyRegistration = () => {
    ...
    if (currentUser.role === Role.ADMIN) {
        ...
        if (selectedDepartmentId) {
            ...
            workersToShow = allWorkers.filter(w => validLocs.includes(w.locationId));
        } else {
            workersToShow = allWorkers;
        }
    } else {
        // Manager sees their workers
        workersToShow = getWorkersByManager(currentUser.id);
    }
  ...
  const generateReportData = useMemo(() => {
    ...
    if (currentUser.role !== Role.ADMIN) {
        ...
        workersToReport = allWorkers.filter(w => managerLocs.includes(w.locationId));
    } else if (selectedDepartmentId) {
        // Admin with filter
        const subLocs = locations.filter(l => l.parentId === selectedDepartmentId).map(l => l.id);
        const validLocs = [selectedDepartmentId, ...subLocs];
        workersToReport = allWorkers.filter(w => validLocs.includes(w.locationId));
    }
